...
def parse_arguments():
    ...
    parser.add_argument("--config", action="store", type=str, default="configs/3.conf",
...
def train_networks(encoder, ca, c_pro_gan, dataset, epochs,
    ...
    fixed_noise = th.randn(len(fixed_captions), c_pro_gan.latent_size - fixed_c_not_hats.shape[-1]).to(device)
...
def main(args):
    ...
    # if not isinstance(text_encoder, PretrainedEncoder):
    #     text_encoder.train()
    ...
    temp_data = dl.get_data_loader(dataset, config.batch_sizes[args.start_depth], num_workers=3)
    fixed_captions, fixed_real_images = iter(temp_data).next()
    ...
    # print(fixed_embeddings.size())

    # # save the fixed_images once:
    # fixed_save_dir = os.path.join(config.sample_dir, "__Real_Info")
